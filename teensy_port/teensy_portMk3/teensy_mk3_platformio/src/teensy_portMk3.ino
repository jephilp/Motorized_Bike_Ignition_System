...
{ 
  ...
  noInterrupts();
  oneDegree = (float(sinceInterrupt)+24.4)/360;
  advance = ((((180  + sparkTimingDegrees)* oneDegree)-3000)); //Calculates the advance
  //Starts secondary spark timer
  sinceInterrupt = 0;
  sparkTimer.begin(sparkStart,advance);
...
void sparkStart()
{
    ...
    if(sparkOn)
  {
   ...
   sparkTimer.begin(sparkFire,dwell);
   sparkTimer.priority(0);  
  }else{
   interrupts();   //Global enable interrupts 
  }
